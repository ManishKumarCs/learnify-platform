...
export default function ReasoningPracticePage() {
  ...
  useEffect(() => {
    const loadTopics = async () => {
      try {
        const res = await fetch("/api/reasoning/topics", { cache: "no-store" })
        if (!res.ok) return
  ...
  const fetchQuestions = async () => {
    ...
    setResult(null)
    try {
      const res = await fetch(`/api/reasoning/getQuestions?topic=${encodeURIComponent(selected)}&limit=${limit}`, { cache: "no-store" })
      const data = await res.json()
  ...
  const onSelect = (qIdx: number, opt: string) => {
  ...
  const submitAttempt = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/reasoning/submitAttempt', {
        method: 'POST',
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-foreground">Reasoning Practice</h1>
        <p className="text-muted-foreground mt-2">Choose a topic and practice reasoning questions.</p>
      </div>
