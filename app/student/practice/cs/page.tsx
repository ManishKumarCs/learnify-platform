...
export default function CSPracticePage() {
  ...
  useEffect(() => {
    const loadTopics = async () => {
      try {
        const res = await fetch("/api/cs/topics", { cache: "no-store" })
        if (!res.ok) return
  ...
  const fetchQuestions = async () => {
    ...
    setResult(null)
    try {
      const res = await fetch(`/api/cs/getQuestions?topic=${encodeURIComponent(selected)}&limit=${limit}` , { cache: "no-store" })
      const data = await res.json()
  ...
  const submitAttempt = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/cs/submitAttempt', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
      const data = await res.json()
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-foreground">CS Concepts Practice</h1>
        <p className="text-muted-foreground mt-2">Choose a topic and practice CS concepts questions.</p>
      </div>
